...
apollo_control = list(
  modelDescr = "D2_Pilot1_MNL_ModelZero",
  modelName  = "D2_Pilot1_MNL_ModelZero", ## Added dates last verified
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['C']]  = asc_C +
    b_Bee * (Insect_PlanC == 2) +
    b_Wasp * (Insect_PlanC == 1) 
...
D2_Pilot1_MNL_ModelZero = apollo_estimate(apollo_beta, 
                                          apollo_fixed, 
                                          apollo_probabilities, 
                                          apollo_inputs)

# Model output and results here alongside saving information
apollo_modelOutput(D2_Pilot1_MNL_ModelZero,
                   modelOutput_settings = list(printPVal = TRUE))



apollo_saveOutput(D2_Pilot1_MNL_ModelZero,saveOutput_settings = list(printPVal=TRUE))
# saveRDS(D2_Pilot1_MNL_ModelZero, here("CEoutput/ModelOne","D2_Pilot1_MNL_ModelZero.rds"))
...
#
nsim <- 1000
betas <- D2_Pilot1_MNL_ModelZero$estimate
vcmat <- D2_Pilot1_MNL_ModelZero$robvarcov
#
#
...
#
wtp <-
  matrix(0, nrow = 5, ncol = 3) # use this matrix to store WTP means and CIs
...
PlotData %>% ggplot(aes(y = variable, 
                        x = mean)) +
  geom_boxplot() +
